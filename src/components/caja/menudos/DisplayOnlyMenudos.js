...
import DisplayNewMenudosAddedToOrders from './DisplayNewMenudosAddedToOrders';

const DisplayOnlyMenudos = () => {
  ...
  return (
    <>
      <div className="container">
        <h1 className="text-center text-2xl font-bold mb-4">Menudos</h1>
        {orders.length <= 0 && (
          ...
          {orders
            ...
            .map((order) => (
              ...
              >
                ...
                >
                  Imprimir Menudos Mesa:{' '}
                  {order.mesa < 1 ? 'Para Llevar' : order.mesa}
                </button>
              </div>
            ))}
        </div>
        <DisplayNewMenudosAddedToOrders />
      </div>
    </>
  );
};

export default DisplayOnlyMenudos;
